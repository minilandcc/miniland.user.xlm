            ...
            return(
                <div className='border rounded-xd p-3 d-flex align-items-center mb-2'>
                   <div className='me-auto'>
                     <p className='text-primary fw-bold m-0'>{item?.asset?.name}</p> 
                     <p className='m-0'>{item?.unit?.name}</p>
                     <p className='m-0 text-uppercase'>{NumberFormat(item?.rate?.nmbr/1000000 || 0, 'w', 2)} {item?.rate?.tick}/sqft</p> 
                    
                   </div>
                   <div className='text-end'> <button onClick={()=> navigate( item.feat.mode == "sale"? `/user/offer/${item.item}` : `/user/resale/offer/${item.item}`) } className='btn btn-primary rounded-xx text-small'>View Details</button> </div>
                </div>
