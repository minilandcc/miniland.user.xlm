...
export default function TransfersActionsMobileModule() {
  ...
  return (
    <>
      <div className="back-color-wite rounded-xd py-3 d-flex flex-wrap gap-2 d-md-none">
        {data &&
          data.map((item, i) =>
            item.actv ? (
              <div
                className={`d-flex p-2 align-middle bg-white border rounded-wd w-100 ${
                  item.live ? "cursor hidark " : "text-color-tint"
                }`}
                style={{ height: "2.8rem" }}
                key={i}
